...
func TestEngineInfer_ProdFile(t *testing.T) {
	...
	if record.Role != model.RoleProd {
		t.Errorf("Role = %v, want prod", record.Role)
	}
}
...
func TestEngineInfer_OverrideTakesPrecedence(t *testing.T) {
	...
	if record.Role != model.RoleProd {
		t.Errorf("Role = %v, want prod (from override)", record.Role)
	}
}
...
func TestEngineInferBatch(t *testing.T) {
	...
	if records[0].Role != model.RoleProd {
		t.Errorf("records[0].Role = %v, want prod", records[0].Role)
	}
