...
func TestReportJSONRoundTrip(t *testing.T) {
	original := Report{
		...
		Responsibilities: []Responsibility{
			{Role: RoleCore, LOC: 30000, Files: 60, Confidence: 0.95},
...
func TestReportWithFiles(t *testing.T) {
	report := Report{
		...
		Languages:        []LanguageComp{},
		Files: []*FileRecord{
			{Path: "main.go", LOC: 100, Role: RoleCore, Confidence: 0.9, Signals: []Signal{SignalPath}},
		},
	}
...
func TestLanguageCompWithResponsibilities(t *testing.T) {
	...
	if decoded.Responsibilities[RoleCore] != 20000 {
		t.Errorf("Responsibilities[core] = %v, want 20000", decoded.Responsibilities[RoleCore])
	}
}
...
func TestRatiosAllFields(t *testing.T) {
	...
	if decoded.TestToCore != 0.67 {
		t.Errorf("TestToCore = %v, want 0.67", decoded.TestToCore)
	}
	if decoded.InfraToCore != 0.10 {
		t.Errorf("InfraToCore = %v, want 0.10", decoded.InfraToCore)
	}
}
