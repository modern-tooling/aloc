...
func TestComputeResponsibilities(t *testing.T) {
	records := []*model.FileRecord{
		{Path: "a.go", LOC: 100, Role: model.RoleProd, Confidence: 0.9},
		{Path: "b.go", LOC: 200, Role: model.RoleProd, Confidence: 0.95},
	...
	if resp[0].Role != model.RoleProd {
		t.Errorf("First role = %v, want prod", resp[0].Role)
	}
	if resp[0].LOC != 300 {
		t.Errorf("Prod LOC = %v, want 300", resp[0].LOC)
	}
	if resp[0].Files != 2 {
...
func TestComputeRatios(t *testing.T) {
	...
	if ratios.TestToProd != 0.5 {
		t.Errorf("TestToProd = %v, want 0.5", ratios.TestToProd)
	}
	if ratios.InfraToProd != 0.2 {
		t.Errorf("InfraToProd = %v, want 0.2", ratios.InfraToProd)
	}
}
...
func TestComputeRatios_ZeroProd(t *testing.T) {
	...
	if ratios.TestToProd != 500 {
		t.Errorf("TestToProd = %v, want 500 (prod=1)", ratios.TestToProd)
	}
}

func TestComputeLanguageBreakdown(t *testing.T) {
	records := []*model.FileRecord{
		{Path: "a.go", LOC: 100, Lines: model.LineMetrics{Code: 100}, Language: "Go", Role: model.RoleProd},
		{Path: "b.go", LOC: 200, Lines: model.LineMetrics{Code: 200}, Language: "Go", Role: model.RoleProd},
		{Path: "a_test.go", LOC: 100, Lines: model.LineMetrics{Code: 100}, Language: "Go", Role: model.RoleTest},
		{Path: "c.ts", LOC: 150, Lines: model.LineMetrics{Code: 150}, Language: "TypeScript", Role: model.RoleProd},
	}
...
func TestComputeLanguageBreakdown_SignificantOnly(t *testing.T) {
	records := []*model.FileRecord{
		{Path: "a.go", LOC: 1000, Lines: model.LineMetrics{Code: 1000}, Language: "Go", Role: model.RoleProd},
...
func TestCompute(t *testing.T) {
	records := []*model.FileRecord{
		{Path: "a.go", LOC: 100, Language: "Go", Role: model.RoleProd, Confidence: 0.9},
...
func TestCompute_ExcludeFiles(t *testing.T) {
	records := []*model.FileRecord{
		{Path: "a.go", LOC: 100, Role: model.RoleProd},
	}
