...
var HybridReductionRates = map[model.Role]struct {
	...
	model.RoleProd:  {0.20, "AI suggests → human implements"},
...
func ComputeQuickActions(responsibilities []model.Responsibility, ratios model.Ratios, human *model.HumanEffort) []model.QuickAction {
	...
	// Find prod LOC for gap calculations
	var prodLOC int
	for _, r := range responsibilities {
		if r.Role == model.RoleProd {
			prodLOC = r.LOC
			break
		}
	}
	...
	if ratios.TestToProd < 0.5 {
		targetRatio := 0.5
		currentTestLOC := int(ratios.TestToProd * float32(prodLOC))
		targetTestLOC := int(targetRatio * float64(prodLOC))
		...
		if human != nil {
			...
			costPerLOC := human.EstimatedCost / float64(prodLOC)
			savings = costPerLOC * float64(locGap) * 0.30
		}

		actions = append(actions, model.QuickAction{
			Priority:    1,
			Description: fmt.Sprintf("Add ~%d LOC of tests to reach 0.5 test/prod ratio", locGap),
			Savings:     savings,
			LOCGap:      locGap,
		})
	}
	...
	// Check documentation
	if ratios.DocsToProd < 0.2 {
		targetRatio := 0.2
		currentDocsLOC := int(ratios.DocsToProd * float32(prodLOC))
		targetDocsLOC := int(targetRatio * float64(prodLOC))
