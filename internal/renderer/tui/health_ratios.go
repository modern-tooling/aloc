...
func RenderHealthRatios(ratios model.Ratios, theme *renderer.Theme) string {
	...
	// Test/Prod ratio
	testHealth := assessTestRatio(ratios.TestToProd)
	b.WriteString(renderRatioLine("Test / Prod", ratios.TestToProd, testHealth, theme))

	// Docs/Prod ratio
	docsHealth := assessDocsRatio(ratios.DocsToProd)
	b.WriteString(renderRatioLine("Docs / Prod", ratios.DocsToProd, docsHealth, theme))

	// Infra/Prod ratio
	infraHealth := assessInfraRatio(ratios.InfraToProd)
	b.WriteString(renderRatioLine("Infra / Prod", ratios.InfraToProd, infraHealth, theme))

	// Config/Prod ratio
	configHealth := assessConfigRatio(ratios.ConfigToProd)
	b.WriteString(renderRatioLine("Config / Prod", ratios.ConfigToProd, configHealth, theme))

	// Generated/Prod ratio
	genHealth := assessGeneratedRatio(ratios.GeneratedToProd)
	b.WriteString(renderRatioLine("Generated / Prod", ratios.GeneratedToProd, genHealth, theme))
...
func RenderHealthRatiosWithGauges(ratios model.Ratios, lines model.LineMetrics, theme *renderer.Theme) string {
	...
	// Test/Prod with gauge
	testHealth := assessTestRatio(ratios.TestToProd)
	b.WriteString(renderRatioWithGauge("Test / Prod", float64(ratios.TestToProd), 0.5, 0.8, testHealth, theme))
	...
	// Docs/Prod with gauge
	docsHealth := assessDocsRatio(ratios.DocsToProd)
	b.WriteString(renderRatioWithGauge("Docs / Prod", float64(ratios.DocsToProd), 0.2, 0.5, docsHealth, theme))

	// Infra/Prod with gauge (lower is better)
	infraHealth := assessInfraRatio(ratios.InfraToProd)
	b.WriteString(renderRatioWithGauge("Infra / Prod", float64(ratios.InfraToProd), 0.0, 0.1, infraHealth, theme))

	// Config/Prod with gauge (lower is better)
	configHealth := assessConfigRatio(ratios.ConfigToProd)
	b.WriteString(renderRatioWithGauge("Config / Prod", float64(ratios.ConfigToProd), 0.0, 0.05, configHealth, theme))
