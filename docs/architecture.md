...
1. **Role-first, language-second** - Semantic classification (prod/test/infra) matters more than language
...
- Calculate key ratios (test/prod, infra/prod, etc.)
...
|------|---------|-------------------|
| `prod` | Production application logic | Default fallback |
...
|-------|------|---------------|
| `semantic.primary` | prod | Neutral foreground |
